<header class="absolute inset-x-0 top-0 z-50 {{ $page->isActive('/') ? '' : 'shadow' }}">
    <nav class="mx-auto flex max-w-7xl items-center justify-between gap-x-6 px-6 py-3 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:gap-x-12">
            ...
            <a title="{{ $item->title }}" href="{{ $item->href }}" id="{{ strtolower($item->title) }}-nav-link" class="text-grey-900 hover:text-grey-700 {{ $page->isActive($item->href) ? 'active font-semibold' : 'font-normal' }}" @if($item->href === '/#pricing') data-umami-event="pricing-click" data-umami-event-source="header-nav" @endif>{{ $item->title }}</a>
            @endforeach
        </div>
        <div class="flex flex-1 items-center justify-end gap-x-6">
            ...
            <a href="{{ $page->appUrl }}/register" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600" data-umami-event="signup-button" data-umami-event-source="header-nav">Sign up</a>
        </div>
