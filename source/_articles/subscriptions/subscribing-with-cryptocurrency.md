---
extends: _layouts.article
ogtype: article
image: https://addy.io/assets/img/help/subscriptions/subscribe-cryptocurrency.png
section: content
title: Subscribing with cryptocurrency
date: 2026-02-13
description: How to subscribe to addy.io with Bitcoin, Monero, Ethereum or other cryptocurrency. NOWPayments, minimum amount, yearly plans and manual payment option.
helpCategories: [subscriptions]
order: 2
---

addy.io accepts **cryptocurrency** for subscriptions (e.g. Bitcoin, Monero, Ethereum). Payments are processed via [NOWPayments](https://nowpayments.io/). There may be a minimum invoice value (e.g. $10) and only yearly plans may be available when paying with crypto. This article explains how it works and how to subscribe with cryptocurrency. For card or PayPal, see [Subscribing with a card or PayPal](/help/subscribing-with-a-card-or-paypal/).

## How it works

1. [Log in](https://app.addy.io) and go to **Subscription** in the navigation menu (or the [subscription page](https://app.addy.io/settings/subscription)).
2. Choose the **yearly** plan (crypto may not be offered for monthly) and then click the **Proceed To Cryptocurrency Payment** button to be redirected to NOWPayments.
3. You will be shown a payment amount and the supported coins. NOWPayments supports many cryptocurrencies, including:
   - **BTC** (Bitcoin)
   - **XMR** (Monero)
   - **ETH** (Ethereum)
   - **LTC** (Litecoin), **DOGE** (Dogecoin), **DCR** (Decred), **XRP** (Ripple), and others.

For a full list of supported coins, see [NOWPayments](https://nowpayments.io/). There is typically a **minimum invoice value** (e.g. $10), so at the moment you can only pay with cryptocurrency for a yearly plan.

You can choose to purchase upto 3 years when subscribing with cryptocurrency.

4. Complete the payment in the wallet or exchange as instructed. Once the payment is confirmed, your subscription is activated. It may take a little while (a couple of hours) for the subscription to be fully confirmed and activated.

<div class="flex justify-center my-8">
  <img class="shadow max-w-full h-auto" src="/assets/img/help/subscriptions/subscribe-cryptocurrency.png" alt="Subscribe with cryptocurrency in addy.io" title="Subscribing with cryptocurrency">
</div>

## Manual cryptocurrency payment

If you prefer your cryptocurrency payment to be processed manually (e.g. for a custom amount), [contact](/contact/) addy.io for further instructions.
